...
/* --------------------------------------------------------- */
/* STRICT 4-DIGIT PADDER (CIPHER VERSION)                   */
/* --------------------------------------------------------- */
function pad4(n) {
	const raw = String(Math.max(0, Math.min(9999, n | 0))).padStart(4, "0");

	/* convert digits → letters */
	let encoded = "";
	for (let i = 0; i < 4; i++) {
		encoded += digitToLetter[raw[i]];
	}
...
/* --------------------------------------------------------- */
/* HUD ENCODER (NOW LETTER-ENCODED)                         */
/* --------------------------------------------------------- */
